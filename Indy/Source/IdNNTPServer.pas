...
begin
  if not Assigned (OnListGroups) then
  begin
    ASender.Reply.NumericCode := 500;
    exit
  end;
...
var
  required : boolean;
begin
  if Assigned(FOnAuth) and (TIdNNTPThread(ASender.Thread).Authenticated = False) then
  begin
    required := True;
    if Assigned (OnAuthRequired) then
      OnAuthRequired (TidNNTPThread (ASender.Thread), ASender.CommandHandler.Command, ASender.UnparsedParams, required);
