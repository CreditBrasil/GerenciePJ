...
begin
  ...
  if error <= 0 then begin
    //Alex - 15/12/2009 - baseado na página: http://www.openssl.org/docs/ssl/SSL_get_error.html
    //Implementei o retorno do possível erro em formato texto.
    error2 := IdSslGetError(fSSL, error);
    case error2 of
      OPENSSL_SSL_ERROR_NONE:
        ErrorStr := 'OPENSSL_SSL_ERROR_NONE: The TLS/SSL I/O operation completed. This result code is returned if and only if ret > 0.';
      OPENSSL_SSL_ERROR_SSL:
        ErrorStr := 'OPENSSL_SSL_ERROR_SSL: A failure in the SSL library occurred, usually a protocol error. The OpenSSL error queue contains more information on the error.';
      OPENSSL_SSL_ERROR_SYSCALL:
        ErrorStr := 'OPENSSL_SSL_ERROR_SYSCALL: Some I/O error occurred. The OpenSSL error queue may contain more information on the error. If the error queue is empty' +
          ' (i.e. ERR_get_error() returns 0), ret can be used to find out more about the error: If ret == 0, an EOF was observed that violates the protocol.' +
      ...
      OPENSSL_SSL_ERROR_WANT_CONNECT:
        ErrorStr := 'OPENSSL_SSL_ERROR_WANT_CONNECT: The operation did not complete; the same TLS/SSL I/O function should be called again later. The underlying BIO was not connected yet to' +
          ' the peer and the call would block in connect()/accept(). The SSL function should be called again when the connection is established. These messages' +
          ' can only appear with a BIO_s_connect() or BIO_s_accept() BIO, respectively. In order to find out, when the connection has been successfully established,' +
          ' on many platforms select() or poll() for writing on the socket file descriptor can be used.';
      OPENSSL_SSL_ERROR_WANT_READ, OPENSSL_SSL_ERROR_WANT_WRITE:
        ErrorStr := 'OPENSSL_SSL_ERROR_WANT_READ, OPENSSL_SSL_ERROR_WANT_WRITE: The operation did not complete; the same TLS/SSL I/O function should be called again later. If, by then, the underlying BIO has data available' +
          ...
          ' i.e. at least part of an TLS/SSL record will be read or written. Note that the retry may again lead to a SSL_ERROR_WANT_READ or SSL_ERROR_WANT_WRITE condition.' +
          ' There is no fixed upper limit for the number of iterations that may be necessary until progress becomes visible at application protocol level.'#13#10#13#10 +
          'For socket BIOs (e.g. when SSL_set_fd() was used), select() or poll() on the underlying socket can be used to find out when the TLS/SSL I/O function should be retried.'#13#10#13#10 +
          'Caveat: Any TLS/SSL I/O function can lead to either of SSL_ERROR_WANT_READ and SSL_ERROR_WANT_WRITE. In particular, SSL_read() or SSL_peek() may want to write' +
          ' data and SSL_write() may want to read data. This is mainly because TLS/SSL handshakes may occur at any time during the protocol (initiated by either the client' +
          ' or the server); SSL_read(), SSL_peek(), and SSL_write() will handle any pending handshakes';
      OPENSSL_SSL_ERROR_WANT_X509_LOOKUP:
        ErrorStr := 'OPENSSL_SSL_ERROR_WANT_X509_LOOKUP: The operation did not complete because an application callback set by SSL_CTX_set_client_cert_cb() has asked to be called again. The TLS/SSL I/O' +
          ' function should be called again later. Details depend on the application.';
      OPENSSL_SSL_ERROR_ZERO_RETURN:
        ErrorStr := 'OPENSSL_SSL_ERROR_ZERO_RETURN: The TLS/SSL connection has been closed. If the protocol version is SSL 3.0 or TLS 1.0, this result code is returned only if a closure alert has' +
          ' occurred in the protocol, i.e. if the connection has been closed cleanly. Note that in this case SSL_ERROR_ZERO_RETURN does not necessarily indicate that the underlying transport has been closed.';
    else
      ErrorStr := '';
    end;
    raise EIdOSSLConnectError.Create(RSSSLConnectError + #13#10 + ErrorStr);
  end;
